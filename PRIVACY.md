...
On iOS, iCloud player-state synchronization is provided by Apple and tied to the iCloud account configured on the device. On Android, Google Play Games Services currently mirrors achievements and leaderboard scores only; it is not used as a full player-state backup. If a platform service is unavailable, core trivia gameplay continues with local progress.
